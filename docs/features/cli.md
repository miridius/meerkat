...
- **stdout**: nothing in normal operation. Phoenix endpoint log
  lines may appear because Logger defaults to `:info` in dev.
- **stderr**: status messages (`meerkat: review UI at ...`,
  auto-approve breadcrumbs, warnings) and — on approve-with-feedback
  / reject — the rendered comment feedback (see
  [decision-flow.md](decision-flow.md)).
